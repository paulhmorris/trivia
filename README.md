...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create trivia-5a0e
  fly apps create trivia-5a0e-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app trivia-5a0e
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app trivia-5a0e-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'trivia-5a0e-staging' does not match app name in config file 'trivia-5a0e'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name trivia-5a0e-db
  fly postgres attach --app trivia-5a0e trivia-5a0e-db

  fly postgres create --name trivia-5a0e-staging-db
  fly postgres attach --app trivia-5a0e-staging trivia-5a0e-staging-db
  ```
